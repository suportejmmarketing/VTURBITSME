...
app.get('/api/info', checkAdmin, (_req, res) => {
  res.json({ hasFfmpeg: HAS_FFMPEG, publicUrl: config.publicUrl, defaults: DEFAULT_SETTINGS });
});
...
app.get('/e/:id.js', (req, res) => {
  ...
  res.type('application/javascript').send(renderPlayerScript(rowToVideo(row), config.publicUrl));
});
...
app.get('/p/:id', (req, res) => {
  ...
  res.type('html').send(renderPlayerHtml(rowToVideo(row), config.publicUrl));
});
...
app.listen(config.port, () => {
  console.log(`\n  VTurb-Self rodando!`);
  console.log(`  Painel:  ${config.publicUrl}/admin`);
